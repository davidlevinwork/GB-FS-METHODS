...
class GB_AFS:
    @staticmethod
    def get_clustering_results(results: dict) -> list:
...
class GB_BC_FS:
    def get_clustering_results(self, results: dict) -> list:
        # Init with the results of the first fold
        final_results = self.init_cluster_results(results=results)

        division_counter = self.init_division_counter(results=results)
        division_cost_counter = self.init_cost_division_counter(results=results)
...
def compile_train_results(classification_results: dict, clustering_results: dict) -> dict:
    compiled_results = {'clustering': {}, 'classification': {}}

    if "GB_AFS" in config.operation_mode:
        compiled_results['clustering'] = GB_AFS().get_clustering_results(clustering_results)
    elif "GB_BC_FS" in config.operation_mode:
        compiled_results['clustering'] = GB_BC_FS().get_clustering_results(clustering_results)

    if config.operation_mode in [str(OPERATION_MODE.FULL_GB_AFS), str(OPERATION_MODE.FULL_GB_BC_FS)]:
        compiled_results['classification'] = get_classification_results(results=classification_results)

    return compiled_results


def get_classification_results(results: dict) -> dict:
    # Init with the results of the first fold
